...
resource "yandex_resourcemanager_folder" "terraform" {
...
resource "yandex_iam_service_account" "iam-service" {
  name        = "iam-service"
  folder_id   = yandex_resourcemanager_folder.terraform.id
  description = "service account from folder k8s-terra"
  depends_on  = [yandex_resourcemanager_folder.terraform]
}
